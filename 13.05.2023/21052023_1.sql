...
(
	...
	constraint fk_parcela_alee foreign key (subparcela_fid) references subparcela(fid) 
);
...
create table if not exists vita_de_vie
(
	fid serial primary key,
	subparcela_fid integer not null,
...
update vita_de_vie set lungime=ST_Length(geom);

select * from vita_de_vie
